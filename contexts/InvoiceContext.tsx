...
const defaultInvoiceContext = {
    ...
    removeFinalPdf: () => {},
    downloadPdf: () => {},
    downloadPdfInLanguage: (language: string) => {},
    printPdf: () => {},
    previewPdfInTab: () => {},
    saveInvoice: () => {},
...
export const InvoiceContextProvider = ({
    ...
    const downloadPdf = () => {
        ...
        if (invoicePdf instanceof Blob && invoicePdf.size > 0) {
            ...
            const sanitizedInvoiceNumber = invoiceNumber.replace(
                /[^a-zA-Z0-9-_]/g,
                "-"
            );
            const sanitizedReceiverName = receiverName.replace(
                /[^a-zA-Z0-9-_]/g,
                "-"
            );
            ...
            a.href = url;
            a.download = `${sanitizedReceiverName}_Invoice-${sanitizedInvoiceNumber}_${language}.pdf`;
    ...
    /**
     * Downloads a PDF file in a specific language.
     * @param {string} language - The language to download the PDF in.
     */
    const downloadPdfInLanguage = async (language: string) => {
        if (getValues) {
            const data = getValues();
            const invoiceNumber = data.details.invoiceNumber;
